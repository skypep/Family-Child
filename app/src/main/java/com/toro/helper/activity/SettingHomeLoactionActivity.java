...
public class SettingHomeLoactionActivity extends ToroActivity implements LocationSource,
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if(safeguardInfo == null) {
            safeguardInfo = new SafeguardInfo();
        }
    ...
    private void save() {
        ...
        safeguardInfo.setLocationInfo(info);
        if(safeguardInfo.getScope() == 0){
            safeguardInfo.setScope(AppConfig.safeguardDefaultScope);
        }
        ConnectManager.getInstance().submitSafeguard(this,phoneNum,safeguardInfo,ToroDataModle.getInstance().getLocalData().getToken());
    ...
    private void init() {
        ...
        aMap.setOnMapLoadedListener(new AMap.OnMapLoadedListener() {
            ...
            public void onMapLoaded() {
                LatLng latLng;
                try{
                    double lat = safeguardInfo.getLocationInfo().getLatLng().latitude;
                    double lng = safeguardInfo.getLocationInfo().getLatLng().longitude;
...
//        Log.i("MY", "onLocationChanged");
        if (mListener != null && amapLocation != null) {
            if (amapLocation != null
                ...
                String errText = getString(R.string.location_error) + amapLocation.getErrorCode()+ ": " + amapLocation.getErrorInfo();
...
//        Log.i("MY", "doSearchQuery");
            ...
            if (poiResult != null && poiResult.getQuery() != null) {
                if (poiResult.getQuery().equals(query)) {
                    ...
                    if (poiItems != null && poiItems.size() > 0) {
                        ...
                        selectedPoiString = "";
                        Toast.makeText(SettingHomeLoactionActivity.this, R.string.search_empty, Toast.LENGTH_SHORT).show();
                    }
                }
            } else {
                selectedPoiString = "";
    ...
    private void updateListview(List<PoiItem> poiItems) {
        ...
        selectedPoiString = resultData.get(selectedIndex).getTitle();
        listView.setSelection(0);
        searchResultAdapter.setSelectedPosition(selectedIndex);
    ...
    AdapterView.OnItemClickListener onItemClickListener = new AdapterView.OnItemClickListener() {
        @Override
        public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
            if (position != searchResultAdapter.getSelectedPosition() && position != 0) {
...
//            locationMarker.startAnimation();
    ...
    public boolean bindData(int tag, Object object) {
        ...
        if(status && tag == ConnectManager.SUBMIT_SAFEGUARD) {
            startActivity(SafeguardSettingActivity.createIntent(this,safeguardInfo,phoneNum));
            finish();
        }
        return status;
    }

    public static Intent createIntent(Context context, String phoneNum, SafeguardInfo safeguardInfo) {
